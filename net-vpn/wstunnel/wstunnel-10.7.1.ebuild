...
SRC_URI="
	https://github.com/erebe/${PN}/archive/refs/tags/v${PV}.tar.gz
		-> ${P}.tar.gz
	${CARGO_CRATE_URIS}
"
...
src_install() {
	...
	insopts -m 0755
	newins "${FILESDIR}"/wrapper.sh wrapper

	if use systemd; then
		systemd_dounit "${FILESDIR}"/${PN}-server.service
		systemd_dounit "${FILESDIR}"/${PN}-client.service
		systemd_newuserunit "${FILESDIR}"/${PN}-server_user.service ${PN}-server.service
		systemd_newuserunit "${FILESDIR}"/${PN}-client_user.service ${PN}-client.service
	fi
}
